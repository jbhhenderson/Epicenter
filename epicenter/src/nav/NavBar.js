...
import { getUser } from "../providers/ApiManager"
...
export const NavBar = () => {
  ...
  useEffect(() => {
    getUser(epicenterUserObject.id)
    .then((foundUser) => {
...
<nav className="bg-white  sm:px-4 py-2.5 dark:bg-black fixed w-full z-20 top-0 left-0 border-b border-gray-200 dark:border-gray-600">
  ...
  <div className="flex md:order-2">
    <div className="text-white pt-2 pr-6">Signed in as: {currentUser.username}</div>
  ...
  <div className="items-center pl-32 justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
